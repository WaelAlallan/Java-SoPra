...
/**
 * Klasse zum erstellen des GUI-Fensters Studien Uebersicht der Study Nurse
 * 
 * @author Sebastian Huck
 */
public class StudienUebersichtStudyNurseFormular {
	...
	private ListViewUndButtons<Studie> sltf;
	private ArrayList<Studie> studien;
	private ButtonRow zurueck;
	 ...
	 */
	public StudienUebersichtStudyNurseFormular(Stage stage, ArrayList<Studie> studien) {
	...
	private void init(ArrayList<Studie> studien) {
		...
		sltf = GUIHelfer.<Studie>getStandardListViewUndButtons(null, null, studien, false, true);
	...
	private void beobachten() {
		...
		buttonRow.getButton("Studien filtern").setOnAction(new EventHandler<ActionEvent>() {
			 ...
			 */
			public void handle(ActionEvent e) {
				StudienFilternStudyNurseFormular sf = new StudienFilternStudyNurseFormular(stage, scene);
			}
		});

		sltf.getButton("Weiter").setOnAction(new EventHandler<ActionEvent>() {
			/**
			 * Methode zur Beschreibung der Aktion in einem anderen Thread
			 * @param e ActionEvent der Aktion
			 */
			public void handle(ActionEvent e) {
				Studie studie = sltf.getSelektiertesObjekt();
				if (studie != null) {
					StudieAnsehenStudyNurseFormular saf = new StudieAnsehenStudyNurseFormular(stage, scene, studie);
				} else {
					GUIHelfer.FehlermeldungFormular("Fehler!", "Es wurde keine Studie angeklickt!",
							"Bitte klicken Sie eine Studie an, um diesen ueber Weiter anzeigen lassen zu koennen.");
				}
			}
		});
	}
