...
/**
 * GUI Klasse zum erstellen des GUI Fensters Kuehlschrank filtern
 * 
 * @author Alexander Leifhelm, Sebastian Huck, Wael Al Allan
 *
 */
public class KuehlschrankFilternFormular {
	private Stage stage;
	 ...
	 */
	public KuehlschrankFilternFormular(Stage stage, Scene oldScene) {
		this.stage = stage;
	 ...
	 */
	void init() {
		kopfUUB = new UeberschriftUndButton("K\u00FChlschr\u00E4nke filtern", "Hauptmen\u00FC", null);
		String[] label1 = { "Name:" };
		String[] label2 = { "Raum:" };
		...
		String[] buttons2 = { "Abbrechen", null, "Filtern" };
	 ...
	 * Raumauswahlmenue und ein KuehlschrankFilter Fenster oeffnen. Bei Abbrechen
	...
	void beobachteAktionen() {
		...
		buttonsBR2.getButton("Abbrechen").setOnAction(event -> {
			...
			KuehlschrankMenueFormular kmf = new KuehlschrankMenueFormular(stage, oldScene, kuehlschraenkeAL);
		});

		buttonsBR2.getButton("Filtern").setOnAction(event -> {
			String name = eingabenSLTF1.getText("Name:");
			String temp = eingabenSLTF3.getText("Eingestellte Temperatur [\u00B0C]:");

			try {
				ArrayList<Kuehlschrank> gefilterteKuehlschraenke = kss.getKuehlschraenke(name, temp, raum);
				KuehlschrankMenueFormular rmf = new KuehlschrankMenueFormular(stage, oldScene,
						gefilterteKuehlschraenke);
