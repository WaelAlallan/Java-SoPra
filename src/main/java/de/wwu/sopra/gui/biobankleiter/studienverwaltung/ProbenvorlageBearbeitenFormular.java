...
/**
 * GUI Klasse zum erstellen des Fensters Probenvorlagenbearbeitenformular
 * 
 * @author Alexander Leifhelm, Sebastian Huck, Wael Al Allan, Cedric Vadder
 */
public class ProbenvorlageBearbeitenFormular {

	private Stage stage;
	private Scene scene;
	private Scene oldScene;
	private ProbenVorlage probenvorlage;
	...
	/**
	 * Dder Konstruktor der Klasse
	 * 
	 * @pre stage darf nicht null sein
	 * @pre oldScene darf nicht null sein
	 * @param stage          das aktive Fenster wird uebergeben
	 * @param oldScene       die alte Scene wird mit uebergeben
	 * @param probenvorlage  die zu bearbeitende Probenvorlage
	 * @param visitenVorlage die zugehoerige Visitenvorlage
	 * @param studie         die zugehoerige Studie
	 */
	public ProbenvorlageBearbeitenFormular(Stage stage, Scene oldScene, ProbenVorlage probenvorlage,
			VisitenVorlage visitenVorlage, Studie studie) {
		this.stage = stage;
	...
	private void init() {
		...
		kopfUUB = new UeberschriftUndButton("Probenvorlage bearbeiten", "Hauptmen\u00FC", null);
		...
		listeLVUBKategorie.setSelektiertesObjekt(probenvorlage.getKategorie());
		...
		listeLVUBBehaeltertyp = GUIHelfer.<ProbenBehaelterTyp>getStandardListViewUndButtons("Beh\u00E4ltertyp:", null,
				typen, false, false);
		listeLVUBBehaeltertyp.setSelektiertesObjekt(probenvorlage.getProbenBehaelterTyp());
			...
			public void handle(ActionEvent e) {
				...
				try {
					if (listeLVUBBehaeltertyp.getSelektiertesObjekt() != null
						...
						VisitenVorlage bearbeitet = pvS.probenVorlageBearbeiten(visitenVorlage, probenvorlage,
		...
		buttonsBR.getButton("L\u00F6schen").setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent e) {
				...
				try {
					...
					alert.setContentText("Sind sie sicher, dass die Probenvorlage gel\u00f6scht werden soll?");
					Optional<ButtonType> result = alert.showAndWait();

					if (result.get() == ButtonType.OK) {
						pvS.probenVorlageLoeschen(visitenVorlage, probenvorlage);
						VisitenvorlageBearbeitenFormular vvBF = new VisitenvorlageBearbeitenFormular(stage, scene,
